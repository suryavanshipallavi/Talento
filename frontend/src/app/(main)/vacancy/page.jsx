...
import default_image from "../../../../public/default-logo.png"
const qualifications = [
  'BCA', 'MCA', 'B.Tech', 'M.Tech', 'B.Com', 'M.Com', 'BBA', 'MBA', 'B.Sc', 'M.Sc'
];
...
const salaryRanges = [
  ...
  '25000-30000', '30000-35000', 'not disclosed'
];
...
const Vacancy = () => {
  ...
  const searchCompany = (e) => {
    ...
    setJobList(
      masterList.filter((job) =>
        job.designation.toLowerCase().includes(val.toLowerCase())
      )
    );
  };
  ...
  const displayJobs = () => {
    return jobList.map((job) => {
      ...
      return (
        <div key={job._id} className='flex flex-wrap'>
          <div className="w-full max-w-md px-8 py-4 mt-16 ml-10 bg-gray-100 rounded-lg shadow-lg dark:bg-gray-800">
            <div className="flex justify-center -mt-16 md:justify-end">
              <Link href={'/company-profile/' + job.company._id}>
                <img
                  className="object-cover w-20 h-20 rounded-full border dark:border-blue-400"
                  alt="Company Logo"
                  src={logoUrl}
                />
              </Link>
    ...
    <div className="flex">
      <aside className="flex flex-col w-64 h-screen px-4 py-8 overflow-y-auto bg-white border-r dark:bg-gray-900 dark:border-gray-700">
        ...
        <div className="relative mt-6">
          ...
          <input
            type="text"
            className="w-full py-2 pl-10 pr-4 text-gray-700 bg-white border rounded-md dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring focus:ring-blue-300 focus:ring-opacity-40"
            placeholder="Search"
        ...
        <div className="flex flex-col justify-between flex-1 mt-6">
          <nav>
            ...
            <div>
              <span className="ml-2 text-black font-semibold">Qualification</span>
              {qualifications.map((qualification) => (
                <label key={qualification} className="flex items-center px-4 py-2">
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setSelQualifications(
                          selQualifications.filter((q) => q !== qualification)
                        );
                      }
                    }}
                  />
            ...
            <div>
              ...
              {salaryRanges.map((salaryRange) => (
                <label key={salaryRange} className="flex items-center px-4 py-2">
                  <input
                    type="checkbox"
                    className="form-checkbox h-5 w-5 text-gray-600"
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setSelSalaryRanges(
                          selSalaryRanges.filter((s) => s !== salaryRange)
                        );
                      }
                    }}
                  />
            ...
            <div>
              ...
              {jobTypes.map((jobType) => (
                <label key={jobType} className="flex items-center px-4 py-2">
                  <input
                    type="checkbox"
                    className="form-checkbox h-5 w-5 text-gray-600"
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setSelJobTypes(
                          selJobTypes.filter((t) => t !== jobType)
                        );
                      }
                    }}
                  />
